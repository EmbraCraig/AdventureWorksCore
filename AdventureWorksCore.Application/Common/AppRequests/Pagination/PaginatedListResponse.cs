...
namespace AdventureWorksCore.Application.Common.AppRequests.Pagination;
    ...
    int PageSize)
{
    ...
    public static async Task<PaginatedListResponse<T>> Create<Tin>(IQueryable<T> source, PaginatedListQuery query, CancellationToken cancellationToken)
    {
        var pageIndex = query.PageIndex;
        var pageSize = query.PageSize;
